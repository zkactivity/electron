...
const MenuItem = function (options) {
  ...
  for (let key in options) {
    if (options.label === 'Paste') {
      console.log(`[Paste] key: ${key} | value: ${options[key]}`)
    }
    else if (options.label === 'Paste (no role)') {
      console.log(`[Paste (no role)] key: ${key} | value: ${options[key]}`)
    }
...
MenuItem.prototype.overrideProperty = function (name, defaultValue = null) {
  if (name === 'enabled') {
    console.log(`enabled?: ${defaultValue}\n`)
  }
  if (this[name] == null) {
...
MenuItem.prototype.overrideReadOnlyProperty = function (name, defaultValue) {
  // if (name === 'enabled') {
  //   console.log(`[ReadOnly] enabled?: ${defaultValue}\n`)
  // }
